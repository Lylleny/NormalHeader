...
- (void)setFirstWaveLayerPath
{
    ...
    for (float x = 0.0f; x <= _waterWaveWidth; x ++) {
        y = _waveAmplitude * sin(_waveCycle * x + _waveOffsetX) + _wavePointY + 10;
...
- (void)setWaveLayerpath{
    ...
    CGFloat y = 150;
    CGPathMoveToPoint(patch, nil, 0, y);
    for (float x = 0; x < _waterWaveWidth; x ++) {
        y = 13*sin(_waveCycle*x + _waveOffsetX + 10)+_wavePointY;
        CGPathAddLineToPoint(patch, nil, x, y);
    }
    
    CGPathAddLineToPoint(patch, nil, _waterWaveWidth, self.frame.size.height);
    CGPathAddLineToPoint(patch, nil, 0, self.frame.size.height);
    CGPathCloseSubpath(patch);
    
    _secondeWaveLayer.path = patch;
    CGPathRelease(patch);
}
...
- (void)setSecondWaveLayerPath
{
    ...
    CGPathMoveToPoint(path, nil, 0, y);
    for (float x = 0.0f; x <= _waterWaveWidth; x ++) {
        y = (_waveAmplitude -2) * sin(_waveCycle * x + _waveOffsetX +15) + _wavePointY ;
        CGPathAddLineToPoint(path, nil, x, y);
    }
